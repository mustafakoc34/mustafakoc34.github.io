...
function durdur() {
    ...
    for (var i = 0; i < box.length; i++) {
        if (box[i]) {
            if (box.length < 15) {
                var rect = box[i].getBoundingClientRect();
                console.log(rect);
                if (rect.left > 680 && rect.left < 720) {
            ...
            } else if (box.length >= 15 && 28 > box.length) {
                var rect = box[i].getBoundingClientRect();
                console.log(rect);
                if (rect.left > 650 && rect.left < 730) {
                    box[i].classList.add("stopBox");
                    console.log(box[i]);
                } else {
                    solAlan.innerHTML = `
